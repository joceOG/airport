<template>
  <v-app id="inspire">
    ...
    <v-app-bar app height="110" color="white">
      <!--  <v-app-bar-nav-icon @click="drawer = !drawer" color="black"></v-app-bar-nav-icon> -->

      <v-img max-height="78" max-width="78" src="./assets/logo.png"></v-img>
      <v-spacer></v-spacer>
      <v-row justify="end" height="" style="margin-right: 10px">
        ...
        <v-menu bottom min-width="200px" rounded offset-y>
          <template v-slot:activator="{ on }">
            <v-btn icon class="ma-3" x-large v-on="on" v-show="e == 1">
              <v-avatar color="brown" size="85">
          ...
          <v-card>
            <v-list-item-content class="justify-center">
              <div class="mx-auto text-center">
                ...
                <v-btn depressed rounded text> PAREMETRES </v-btn>
                <v-divider class="my-3"></v-divider>
                <v-btn depressed rounded text @click="getLoggout()"> DECONNEXION </v-btn>
              </div>
            </v-list-item-content>
          </v-card>
        </v-menu>
    ...
    <v-container class="fill-height" style="width: 100px; padding: 0px" fluid>
      <router-view />
    </v-container>
    <v-footer color="primary darken-2" app>
      <v-container class="pa-0" fluid>
        <v-row dense>
          ...
          <v-col class="pa-0">
            <span
              class="white--text text-caption d-flex flex-row-reverse"
              stye="position:absolute; right:20px"
              >v{{ version }}</span
            >
          </v-col>
        </v-row>
...
export default class layout extends Vue {
 ...
 async getLoggout() {
       try {
          const result = await axios.post("http://127.0.0.1:8000/api/user/logout");

<template>
  <v-container fluid class="fapp" style="padding: 0px">
    ...
    >
      <p class="" style="text-align: center; margin-top: 140px">
        <b><h1 style="font-size: 65; color: rgb(0, 0, 51)">Koli&Co</h1></b>
      </p>
      <p
        class=""
        style="margin-top: -10px; text-align: center; color: rgb(0, 0, 51); font-size: 25"
      >
        Le service d'expédition par vous et pour vous
      </p>
      <p class="" style="text-align: center">
      <v-row>
      <<v-col>
        <v-btn width="550" color="white" dark>
          <h4 style="text-align: center; color: rgb(0, 0, 51)" rounded>ENVOYER UN KOLI</h4>
        </v-btn>
      </v-col>
      <v-col>
        <v-btn width="550" color="white" dark>
          <h4 style="text-align: center; color: rgb(0, 0, 51)" rounded>TRANSPORTER UN KOLI</h4>
        </v-btn>
      </v-col>
      </v-row>
    ...
    <v-img
      style="width: 100%; padding: 0px; height: 350"
    ...
    <v-dialog v-model="dialog" width="500">
      <v-card>
        ...
        <v-btn class="ma-2 centrer" color="primary" @click="signin"> Sign In </v-btn>

        <center>
          <p class="">Vous n'avez pas de compte? <a>S'INSCRIRE</a></p>
        </center>
        <v-divider></v-divider>
    ...
    <v-dialog v-model="dialog2" width="500">
      <v-card>
        ...
        <v-btn class="ma-2 centrer" color="primary" @click="signup"> Mot de Passe </v-btn>
      </v-card>
    </v-dialog>
  </v-container>
</template>
  ...
  message(payload: any, x: any) {
    if (x == 1) {
      if ((payload.status = true)) {
        ...
        this.notif = "Inscription Echouée";
    ...
    if (x == 2) {
      if ((payload.status = true)) {
        ...
        EventBus.$emit('logged', payload)
      } else {
        this.notif = payload.message;
        this.typealert = "error";
        this.showalert = true;
        setTimeout(() => {
          this.showalert = false;
